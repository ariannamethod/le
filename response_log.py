"""
Objectivity - Мини-динамические веса для LE через веб-поиск

Асинхронно ищет контекст в интернете для обогащения ответов LE.
Создает небольшое окно знаний (10-20 строк) для влияния на генерацию.
"""
...
class ObjectivitySearch:
    ...
    async def search_context(self, user_query: str) -> Dict[str, any]:
        ...
        print(f"📊 Context: {len(context_lines)} lines, influence: {influence_strength:.2f}, words: {context_words}")
    ...
    def log_context_for_training(self, context_lines: List[str], user_query: str) -> None:
        """Сохраняет найденный контекст для дообучения LE."""
        if not context_lines:
            return
...
def search_objectivity_sync(user_query: str) -> Dict[str, any]:
    """Синхронная обертка для использования в LE."""
    try:
