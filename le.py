...
def generate(model, idx, max_new_tokens, temperature=1.0, do_sample=False, top_k=None, top_p=None):
    ...
    memory_text = " ".join(memory_messages) if memory_messages else ""
    memory_tokens = _encode(memory_text)
    ...
    # Ограничиваем контекст размером блока
        ...
        if os.path.isfile(fname):
            with open(fname, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    if line:
                        words.append(line)
    chars = sorted(list(set(''.join(words)))) if words else []
...
if __name__ == '__main__':
    ...
    if args.sample_only:
        ...
        if args.prompt:
            sample = sample_prompt(
                ...
                dataset=train_dataset,
                memory=memory,
                max_new_tokens=train_dataset.get_output_length(),
                temperature=args.temperature,
                top_k=top_k
            )
            print(sample)
    ...
    if not args.sample_only and not args.resume and len(sys.argv) == 1:
