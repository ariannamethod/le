...
class CausalBoW(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.register_buffer(
            "bias",
            torch.tril(torch.ones(config.block_size, config.block_size)).view(1, config.block_size, config.block_size),
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--max-steps", type=int, default=-1, help="max number of optimization steps to run for, or -1 for infinite."
    )
    parser.add_argument(
        "--device", type=str, default="cpu", help="device to use for compute, examples: cpu|cuda|cuda:2|mps"
    )
    ...
    # model
    parser.add_argument(
        "--type", type=str, default="transformer", help="model class type to use, bigram|mlp|rnn|gru|bow|transformer"
    )
    ...
    optimizer = torch.optim.AdamW(
        model.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay, betas=(0.9, 0.99), eps=1e-8
    )
